...
[[ch14-2023.pdf#page=12]] This show the core idea of the file system in memory. Each process has open-file table, which points to the system-wide open-file table which points to blocks in memory. But these are cached once they are touched, pulling them into memory, and if accessed again it is much faster.
...
If you know you will need more blocks, you could fx make an extent that is 20 blocks big. This is a possibility to do in theses sytems

# Free space management
Each sector can just contain a bit whether in use or not.
