...
even tho assembly code may be smaller in terms of size, in modern operating system size is not an issue, therefore preference is higher level languages.
...
Debugging is from low to high, since we assume hardware functions, if the layer above fails then it must be there the flaw is. It hides the implementation of from the above layer.
...
GRUB  is an opensource bootstrapping program

Both will load the kernel into memory, can also run diagnostics

2. The kernel is loaded into memory and started.
3. The kernel initializes hardware.
Using temporary RAM (initramfs)location to startup initial drivers and file system 
