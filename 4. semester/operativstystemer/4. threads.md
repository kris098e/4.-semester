...
unable to scale well, since if one thread make system-call the entire process will block
...
There one-to-one is mostly used
...
Variables which are global are shared among all the threads.
...
class for user-initiated, more work than the one above, however it does not have to executed immediately.
